<?php
/**
 * ESP32 Engine — Phase 2 Guide Importer
 * Usage: php import-phase2-guides.php [--dry-run] [--verbose]
 */
...
$guides = array_merge(
    require __DIR__ . '/phase2-data-1.php',
    require __DIR__ . '/phase2-data-2.php'
);
...
foreach ( $guides as $g ) {
    ...
    if ( $dry_run ) {
        echo ( $is_update ? '  – UPDATE' : '  + CREATE' ) . " $slug\n";
    ...
    $wc     = str_word_count( strip_tags( $body_html ) );
    $action = $is_update ? 'Updated' : 'Imported';
    $stats[ $is_update ? 'updated' : 'imported' ]++;
    echo "  ✓ $action [$id] $slug ({$wc} words, " . count($faqs) . " FAQs)\n";
}
...
@file_get_contents($arch, false, stream_context_create(['http'=>['timeout'=>6,'ignore_errors'=>true]]));
preg_match('/HTTP\/\d\.\d (\d{3})/', $http_response_header[0] ?? '', $m);
$acode = (int)($m[1] ?? 0);
...
echo "  URL PASS : $pass / " . count($slugs) . "\n";
echo "\nDone.\n";
