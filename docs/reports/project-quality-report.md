# Project Quality Report

Generated: 2026-07-17T21:23:52Z

Benchmark: **ESP32 Mini Weather Station** is the golden standard (`content/projects/esp32-iot-weather-station.yaml`).

Portfolio: 49 golden · 1 staged placeholder(s)

Standard: [GOLDEN_PROJECT_STANDARD.md](../editorial/GOLDEN_PROJECT_STANDARD.md)

Run `py tools/validate_project_quality.py` to refresh this report.

## Golden Benchmark (ESP32 Mini Weather Station)

- Overall: 100/100
- Child: 100 · Parent: 100 · Teacher: 100
- Engineering: 100 · SEO: 100 · Accessibility: 100

## Summary

| Project | Type | Overall | Priority | Missing required |
|---------|------|---------|----------|------------------|
| ESP32 TinyML Sound Classifier | staged | 84/100 | HIGH | 0 |
| Build a BLE Beacon | golden | 96/100 | LOW | 0 |
| ESP32-CAM Local Network Camera Demo | golden | 97/100 | LOW | 0 |
| Build a Smart Street Light | golden | 97/100 | LOW | 0 |
| Build a Color Object Presence Demo | golden | 98/100 | LOW | 0 |
| Build an ESP32-CAM QR Scanner | golden | 98/100 | LOW | 0 |
| Build a Touch-Key Digital Piano | golden | 98/100 | LOW | 0 |
| Build a Smoke Sensor Alert Demo | golden | 98/100 | LOW | 0 |
| Build an LED Matrix Display | golden | 98/100 | LOW | 0 |
| Build Your OLED Weather Clock | golden | 98/100 | LOW | 0 |
| Build Your Smart Door Lock | golden | 98/100 | LOW | 0 |
| Build a Low-Voltage Energy Meter | golden | 98/100 | LOW | 0 |
| Build a Mailbox Open Alert | golden | 98/100 | LOW | 0 |
| Build Your Smart Thermostat | golden | 98/100 | LOW | 0 |
| Build a Vibration Threshold Monitor | golden | 98/100 | LOW | 0 |
| Build an Indoor Air Quality Monitor | golden | 99/100 | LOW | 0 |
| Build a Camera Capture Server | golden | 99/100 | LOW | 0 |
| Build an MQTT Sensor Dashboard | golden | 99/100 | LOW | 0 |
| Build a Soil Moisture Monitor | golden | 99/100 | LOW | 0 |
| esp32-ac-power-monitor | golden | 100/100 | LOW | 0 |
| Build a Local Face Detection Demo | golden | 100/100 | LOW | 0 |
| esp32-cnc-controller | golden | 100/100 | LOW | 0 |
| Build a Distance Alert Meter | golden | 100/100 | LOW | 0 |
| Build an Educational ECG Waveform Viewer | golden | 100/100 | LOW | 0 |
| Build an APDS-9960 Gesture Control Demo | golden | 100/100 | LOW | 0 |
| Build a Local NEO-6M GPS Reader | golden | 100/100 | LOW | 0 |
| esp32-greenhouse-automation-controller | golden | 100/100 | LOW | 0 |
| Build Your Home Climate Automation | golden | 100/100 | LOW | 0 |
| Build Your IoT Weather Station | golden | 100/100 | LOW | 0 |
| Build an ESP32 IR Learning Remote Demo | golden | 100/100 | LOW | 0 |
| Build a Classroom ESP32 Trainer | golden | 100/100 | LOW | 0 |
| esp32-lightning-detector | golden | 100/100 | LOW | 0 |
| Build Your Line Following Robot | golden | 100/100 | LOW | 0 |
| Build a LoRa Point-to-Point Sensor Link | golden | 100/100 | LOW | 0 |
| Build a Machine Monitoring Node | golden | 100/100 | LOW | 0 |
| Build Your Motion Security Alert | golden | 100/100 | LOW | 0 |
| esp32-neopixel-music-visualizer | golden | 100/100 | LOW | 0 |
| esp32-pulse-oximeter-logger | golden | 100/100 | LOW | 0 |
| Build Your RFID Access Control System | golden | 100/100 | LOW | 0 |
| Build an RFID Inventory Tracker | golden | 100/100 | LOW | 0 |
| esp32-rgb-led-pattern-controller | golden | 100/100 | LOW | 0 |
| Build a 3-DOF Educational Servo Arm | golden | 100/100 | LOW | 0 |
| Build Your Smart Irrigation System | golden | 100/100 | LOW | 0 |
| Build a Smart Parking Sensor | golden | 100/100 | LOW | 0 |
| esp32-smart-power-strip | golden | 100/100 | LOW | 0 |
| Build an Educational Soil pH Monitor | golden | 100/100 | LOW | 0 |
| Build an Estimated UV Index Monitor | golden | 100/100 | LOW | 0 |
| Build a Sound-Triggered Relay Demo | golden | 100/100 | LOW | 0 |
| Build a Water Leak Detector | golden | 100/100 | LOW | 0 |
| Build Your WiFi Robot Controller | golden | 100/100 | LOW | 0 |

## ESP32 TinyML Sound Classifier (`esp32-tinyml-sound-classifier`)

**Type:** staged · **Overall score:** 84/100 · **Priority:** HIGH

### Scores

- Child Experience: 90/100
- Parent Experience: 85/100
- Teacher Experience: 70/100
- Engineering: 80/100
- SEO: 100/100
- Accessibility: 80/100

### Missing sections

- None

### Recommendations

- Upgrade to format: golden — copy content/project-template.yaml and fill all project sections.
- Use esp32-iot-weather-station.yaml as the reference benchmark.

### Warnings

- Staged placeholder — upgrade to format: golden with full project block

## Build a BLE Beacon (`esp32-ble-beacon`)

**Type:** golden · **Overall score:** 96/100 · **Priority:** LOW

### Scores

- Child Experience: 100/100
- Parent Experience: 100/100
- Teacher Experience: 100/100
- Engineering: 87/100
- SEO: 100/100
- Accessibility: 88/100

### Missing sections

- None

### Recommendations

- Minor polish only — align with Mini Weather Station depth where applicable.

## ESP32-CAM Local Network Camera Demo (`esp32-security-camera-system`)

**Type:** golden · **Overall score:** 97/100 · **Priority:** LOW

### Scores

- Child Experience: 100/100
- Parent Experience: 83/100
- Teacher Experience: 100/100
- Engineering: 100/100
- SEO: 100/100
- Accessibility: 100/100

### Missing sections

- None

### Recommendations

- Minor polish only — align with Mini Weather Station depth where applicable.

## Build a Smart Street Light (`esp32-smart-street-light`)

**Type:** golden · **Overall score:** 97/100 · **Priority:** LOW

### Scores

- Child Experience: 93/100
- Parent Experience: 100/100
- Teacher Experience: 100/100
- Engineering: 100/100
- SEO: 100/100
- Accessibility: 88/100

### Missing sections

- None

### Recommendations

- Minor polish only — align with Mini Weather Station depth where applicable.

## Build a Color Object Presence Demo (`esp32-ai-object-detector`)

**Type:** golden · **Overall score:** 98/100 · **Priority:** LOW

### Scores

- Child Experience: 100/100
- Parent Experience: 90/100
- Teacher Experience: 100/100
- Engineering: 100/100
- SEO: 100/100
- Accessibility: 100/100

### Missing sections

- None

### Recommendations

- Minor polish only — align with Mini Weather Station depth where applicable.

## Build an ESP32-CAM QR Scanner (`esp32-cam-qr-scanner`)

**Type:** golden · **Overall score:** 98/100 · **Priority:** LOW

### Scores

- Child Experience: 100/100
- Parent Experience: 100/100
- Teacher Experience: 100/100
- Engineering: 87/100
- SEO: 100/100
- Accessibility: 100/100

### Missing sections

- None

### Recommendations

- Minor polish only — align with Mini Weather Station depth where applicable.

## Build a Touch-Key Digital Piano (`esp32-digital-piano`)

**Type:** golden · **Overall score:** 98/100 · **Priority:** LOW

### Scores

- Child Experience: 100/100
- Parent Experience: 100/100
- Teacher Experience: 100/100
- Engineering: 100/100
- SEO: 100/100
- Accessibility: 88/100

### Missing sections

- None

### Recommendations

- Minor polish only — align with Mini Weather Station depth where applicable.

## Build a Smoke Sensor Alert Demo (`esp32-fire-alarm-system`)

**Type:** golden · **Overall score:** 98/100 · **Priority:** LOW

### Scores

- Child Experience: 100/100
- Parent Experience: 100/100
- Teacher Experience: 100/100
- Engineering: 100/100
- SEO: 100/100
- Accessibility: 88/100

### Missing sections

- None

### Recommendations

- Minor polish only — align with Mini Weather Station depth where applicable.

## Build an LED Matrix Display (`esp32-led-matrix-display`)

**Type:** golden · **Overall score:** 98/100 · **Priority:** LOW

### Scores

- Child Experience: 100/100
- Parent Experience: 100/100
- Teacher Experience: 100/100
- Engineering: 100/100
- SEO: 100/100
- Accessibility: 88/100

### Missing sections

- None

### Recommendations

- Minor polish only — align with Mini Weather Station depth where applicable.

## Build Your OLED Weather Clock (`esp32-oled-weather-clock`)

**Type:** golden · **Overall score:** 98/100 · **Priority:** LOW

### Scores

- Child Experience: 100/100
- Parent Experience: 100/100
- Teacher Experience: 100/100
- Engineering: 100/100
- SEO: 100/100
- Accessibility: 88/100

### Missing sections

- None

### Recommendations

- Minor polish only — align with Mini Weather Station depth where applicable.

## Build Your Smart Door Lock (`esp32-smart-door-lock`)

**Type:** golden · **Overall score:** 98/100 · **Priority:** LOW

### Scores

- Child Experience: 100/100
- Parent Experience: 100/100
- Teacher Experience: 100/100
- Engineering: 100/100
- SEO: 100/100
- Accessibility: 88/100

### Missing sections

- None

### Recommendations

- Minor polish only — align with Mini Weather Station depth where applicable.

## Build a Low-Voltage Energy Meter (`esp32-smart-energy-meter`)

**Type:** golden · **Overall score:** 98/100 · **Priority:** LOW

### Scores

- Child Experience: 100/100
- Parent Experience: 100/100
- Teacher Experience: 100/100
- Engineering: 100/100
- SEO: 100/100
- Accessibility: 88/100

### Missing sections

- None

### Recommendations

- Minor polish only — align with Mini Weather Station depth where applicable.

## Build a Mailbox Open Alert (`esp32-smart-mailbox`)

**Type:** golden · **Overall score:** 98/100 · **Priority:** LOW

### Scores

- Child Experience: 100/100
- Parent Experience: 100/100
- Teacher Experience: 100/100
- Engineering: 100/100
- SEO: 100/100
- Accessibility: 88/100

### Missing sections

- None

### Recommendations

- Minor polish only — align with Mini Weather Station depth where applicable.

## Build Your Smart Thermostat (`esp32-smart-thermostat`)

**Type:** golden · **Overall score:** 98/100 · **Priority:** LOW

### Scores

- Child Experience: 100/100
- Parent Experience: 100/100
- Teacher Experience: 100/100
- Engineering: 100/100
- SEO: 100/100
- Accessibility: 88/100

### Missing sections

- None

### Recommendations

- Minor polish only — align with Mini Weather Station depth where applicable.

## Build a Vibration Threshold Monitor (`esp32-vibration-monitor`)

**Type:** golden · **Overall score:** 98/100 · **Priority:** LOW

### Scores

- Child Experience: 100/100
- Parent Experience: 100/100
- Teacher Experience: 100/100
- Engineering: 100/100
- SEO: 100/100
- Accessibility: 88/100

### Missing sections

- None

### Recommendations

- Minor polish only — align with Mini Weather Station depth where applicable.

## Build an Indoor Air Quality Monitor (`esp32-air-quality-monitor`)

**Type:** golden · **Overall score:** 99/100 · **Priority:** LOW

### Scores

- Child Experience: 93/100
- Parent Experience: 100/100
- Teacher Experience: 100/100
- Engineering: 100/100
- SEO: 100/100
- Accessibility: 100/100

### Missing sections

- None

### Recommendations

- Minor polish only — align with Mini Weather Station depth where applicable.

## Build a Camera Capture Server (`esp32-camera-capture-server`)

**Type:** golden · **Overall score:** 99/100 · **Priority:** LOW

### Scores

- Child Experience: 93/100
- Parent Experience: 100/100
- Teacher Experience: 100/100
- Engineering: 100/100
- SEO: 100/100
- Accessibility: 100/100

### Missing sections

- None

### Recommendations

- Minor polish only — align with Mini Weather Station depth where applicable.

## Build an MQTT Sensor Dashboard (`esp32-mqtt-sensor-dashboard`)

**Type:** golden · **Overall score:** 99/100 · **Priority:** LOW

### Scores

- Child Experience: 93/100
- Parent Experience: 100/100
- Teacher Experience: 100/100
- Engineering: 100/100
- SEO: 100/100
- Accessibility: 100/100

### Missing sections

- None

### Recommendations

- Minor polish only — align with Mini Weather Station depth where applicable.

## Build a Soil Moisture Monitor (`esp32-soil-moisture-monitor`)

**Type:** golden · **Overall score:** 99/100 · **Priority:** LOW

### Scores

- Child Experience: 93/100
- Parent Experience: 100/100
- Teacher Experience: 100/100
- Engineering: 100/100
- SEO: 100/100
- Accessibility: 100/100

### Missing sections

- None

### Recommendations

- Minor polish only — align with Mini Weather Station depth where applicable.

## Build an Isolated AC Current Monitor Prototype (`esp32-ac-power-monitor`)

**Type:** golden · **Overall score:** 100/100 · **Priority:** LOW

### Scores

- Child Experience: 100/100
- Parent Experience: 100/100
- Teacher Experience: 100/100
- Engineering: 100/100
- SEO: 100/100
- Accessibility: 100/100

### Missing sections

- None

### Recommendations

- Minor polish only — align with Mini Weather Station depth where applicable.

## Build a Local Face Detection Demo (`esp32-cam-face-detection`)

**Type:** golden · **Overall score:** 100/100 · **Priority:** LOW

### Scores

- Child Experience: 100/100
- Parent Experience: 100/100
- Teacher Experience: 100/100
- Engineering: 100/100
- SEO: 100/100
- Accessibility: 100/100

### Missing sections

- None

### Recommendations

- Minor polish only — align with Mini Weather Station depth where applicable.

## Build a Low-Voltage Motion Controller Demo (`esp32-cnc-controller`)

**Type:** golden · **Overall score:** 100/100 · **Priority:** LOW

### Scores

- Child Experience: 100/100
- Parent Experience: 100/100
- Teacher Experience: 100/100
- Engineering: 100/100
- SEO: 100/100
- Accessibility: 100/100

### Missing sections

- None

### Recommendations

- Minor polish only — align with Mini Weather Station depth where applicable.

## Build a Distance Alert Meter (`esp32-distance-monitoring-system`)

**Type:** golden · **Overall score:** 100/100 · **Priority:** LOW

### Scores

- Child Experience: 100/100
- Parent Experience: 100/100
- Teacher Experience: 100/100
- Engineering: 100/100
- SEO: 100/100
- Accessibility: 100/100

### Missing sections

- None

### Recommendations

- Minor polish only — align with Mini Weather Station depth where applicable.

## Build an Educational ECG Waveform Viewer (`esp32-ecg-monitor`)

**Type:** golden · **Overall score:** 100/100 · **Priority:** LOW

### Scores

- Child Experience: 100/100
- Parent Experience: 100/100
- Teacher Experience: 100/100
- Engineering: 100/100
- SEO: 100/100
- Accessibility: 100/100

### Missing sections

- None

### Recommendations

- Minor polish only — align with Mini Weather Station depth where applicable.

## Build an APDS-9960 Gesture Control Demo (`esp32-gesture-recognition`)

**Type:** golden · **Overall score:** 100/100 · **Priority:** LOW

### Scores

- Child Experience: 100/100
- Parent Experience: 100/100
- Teacher Experience: 100/100
- Engineering: 100/100
- SEO: 100/100
- Accessibility: 100/100

### Missing sections

- None

### Recommendations

- Minor polish only — align with Mini Weather Station depth where applicable.

## Build a Local NEO-6M GPS Reader (`esp32-gps-tracker`)

**Type:** golden · **Overall score:** 100/100 · **Priority:** LOW

### Scores

- Child Experience: 100/100
- Parent Experience: 100/100
- Teacher Experience: 100/100
- Engineering: 100/100
- SEO: 100/100
- Accessibility: 100/100

### Missing sections

- None

### Recommendations

- Minor polish only — align with Mini Weather Station depth where applicable.

## Build a Low-Voltage Greenhouse Vent Controller (`esp32-greenhouse-automation-controller`)

**Type:** golden · **Overall score:** 100/100 · **Priority:** LOW

### Scores

- Child Experience: 100/100
- Parent Experience: 100/100
- Teacher Experience: 100/100
- Engineering: 100/100
- SEO: 100/100
- Accessibility: 100/100

### Missing sections

- None

### Recommendations

- Minor polish only — align with Mini Weather Station depth where applicable.

## Build Your Home Climate Automation (`esp32-home-climate-automation`)

**Type:** golden · **Overall score:** 100/100 · **Priority:** LOW

### Scores

- Child Experience: 100/100
- Parent Experience: 100/100
- Teacher Experience: 100/100
- Engineering: 100/100
- SEO: 100/100
- Accessibility: 100/100

### Missing sections

- None

### Recommendations

- Minor polish only — align with Mini Weather Station depth where applicable.

## Build Your IoT Weather Station (`esp32-iot-weather-station`)

**Type:** golden · **Overall score:** 100/100 · **Priority:** LOW

### Scores

- Child Experience: 100/100
- Parent Experience: 100/100
- Teacher Experience: 100/100
- Engineering: 100/100
- SEO: 100/100
- Accessibility: 100/100

### Missing sections

- None

### Recommendations

- Minor polish only — align with Mini Weather Station depth where applicable.

## Build an ESP32 IR Learning Remote Demo (`esp32-ir-remote-control`)

**Type:** golden · **Overall score:** 100/100 · **Priority:** LOW

### Scores

- Child Experience: 100/100
- Parent Experience: 100/100
- Teacher Experience: 100/100
- Engineering: 100/100
- SEO: 100/100
- Accessibility: 100/100

### Missing sections

- None

### Recommendations

- Minor polish only — align with Mini Weather Station depth where applicable.

## Build a Classroom ESP32 Trainer (`esp32-learning-trainer`)

**Type:** golden · **Overall score:** 100/100 · **Priority:** LOW

### Scores

- Child Experience: 100/100
- Parent Experience: 100/100
- Teacher Experience: 100/100
- Engineering: 100/100
- SEO: 100/100
- Accessibility: 100/100

### Missing sections

- None

### Recommendations

- Minor polish only — align with Mini Weather Station depth where applicable.

## Build an Educational Lightning Sensor Demo (`esp32-lightning-detector`)

**Type:** golden · **Overall score:** 100/100 · **Priority:** LOW

### Scores

- Child Experience: 100/100
- Parent Experience: 100/100
- Teacher Experience: 100/100
- Engineering: 100/100
- SEO: 100/100
- Accessibility: 100/100

### Missing sections

- None

### Recommendations

- Minor polish only — align with Mini Weather Station depth where applicable.

## Build Your Line Following Robot (`esp32-line-following-robot`)

**Type:** golden · **Overall score:** 100/100 · **Priority:** LOW

### Scores

- Child Experience: 100/100
- Parent Experience: 100/100
- Teacher Experience: 100/100
- Engineering: 100/100
- SEO: 100/100
- Accessibility: 100/100

### Missing sections

- None

### Recommendations

- Minor polish only — align with Mini Weather Station depth where applicable.

## Build a LoRa Point-to-Point Sensor Link (`esp32-lora-remote-sensor-node`)

**Type:** golden · **Overall score:** 100/100 · **Priority:** LOW

### Scores

- Child Experience: 100/100
- Parent Experience: 100/100
- Teacher Experience: 100/100
- Engineering: 100/100
- SEO: 100/100
- Accessibility: 100/100

### Missing sections

- None

### Recommendations

- Minor polish only — align with Mini Weather Station depth where applicable.

## Build a Machine Monitoring Node (`esp32-machine-monitoring-node`)

**Type:** golden · **Overall score:** 100/100 · **Priority:** LOW

### Scores

- Child Experience: 100/100
- Parent Experience: 100/100
- Teacher Experience: 100/100
- Engineering: 100/100
- SEO: 100/100
- Accessibility: 100/100

### Missing sections

- None

### Recommendations

- Minor polish only — align with Mini Weather Station depth where applicable.

## Build Your Motion Security Alert (`esp32-motion-security-alert`)

**Type:** golden · **Overall score:** 100/100 · **Priority:** LOW

### Scores

- Child Experience: 100/100
- Parent Experience: 100/100
- Teacher Experience: 100/100
- Engineering: 100/100
- SEO: 100/100
- Accessibility: 100/100

### Missing sections

- None

### Recommendations

- Minor polish only — align with Mini Weather Station depth where applicable.

## Build an Amplitude-Reactive NeoPixel Visualizer (`esp32-neopixel-music-visualizer`)

**Type:** golden · **Overall score:** 100/100 · **Priority:** LOW

### Scores

- Child Experience: 100/100
- Parent Experience: 100/100
- Teacher Experience: 100/100
- Engineering: 100/100
- SEO: 100/100
- Accessibility: 100/100

### Missing sections

- None

### Recommendations

- Minor polish only — align with Mini Weather Station depth where applicable.

## Build an Educational Pulse Oximeter Logger (`esp32-pulse-oximeter-logger`)

**Type:** golden · **Overall score:** 100/100 · **Priority:** LOW

### Scores

- Child Experience: 100/100
- Parent Experience: 100/100
- Teacher Experience: 100/100
- Engineering: 100/100
- SEO: 100/100
- Accessibility: 100/100

### Missing sections

- None

### Recommendations

- Minor polish only — align with Mini Weather Station depth where applicable.

## Build Your RFID Access Control System (`esp32-rfid-access-control-system`)

**Type:** golden · **Overall score:** 100/100 · **Priority:** LOW

### Scores

- Child Experience: 100/100
- Parent Experience: 100/100
- Teacher Experience: 100/100
- Engineering: 100/100
- SEO: 100/100
- Accessibility: 100/100

### Missing sections

- None

### Recommendations

- Minor polish only — align with Mini Weather Station depth where applicable.

## Build an RFID Inventory Tracker (`esp32-rfid-inventory-tracker`)

**Type:** golden · **Overall score:** 100/100 · **Priority:** LOW

### Scores

- Child Experience: 100/100
- Parent Experience: 100/100
- Teacher Experience: 100/100
- Engineering: 100/100
- SEO: 100/100
- Accessibility: 100/100

### Missing sections

- None

### Recommendations

- Minor polish only — align with Mini Weather Station depth where applicable.

## Build a Single RGB LED Pattern Controller (`esp32-rgb-led-pattern-controller`)

**Type:** golden · **Overall score:** 100/100 · **Priority:** LOW

### Scores

- Child Experience: 100/100
- Parent Experience: 100/100
- Teacher Experience: 100/100
- Engineering: 100/100
- SEO: 100/100
- Accessibility: 100/100

### Missing sections

- None

### Recommendations

- Minor polish only — align with Mini Weather Station depth where applicable.

## Build a 3-DOF Educational Servo Arm (`esp32-robot-arm-controller`)

**Type:** golden · **Overall score:** 100/100 · **Priority:** LOW

### Scores

- Child Experience: 100/100
- Parent Experience: 100/100
- Teacher Experience: 100/100
- Engineering: 100/100
- SEO: 100/100
- Accessibility: 100/100

### Missing sections

- None

### Recommendations

- Minor polish only — align with Mini Weather Station depth where applicable.

## Build Your Smart Irrigation System (`esp32-smart-irrigation-system`)

**Type:** golden · **Overall score:** 100/100 · **Priority:** LOW

### Scores

- Child Experience: 100/100
- Parent Experience: 100/100
- Teacher Experience: 100/100
- Engineering: 100/100
- SEO: 100/100
- Accessibility: 100/100

### Missing sections

- None

### Recommendations

- Minor polish only — align with Mini Weather Station depth where applicable.

## Build a Smart Parking Sensor (`esp32-smart-parking-sensor`)

**Type:** golden · **Overall score:** 100/100 · **Priority:** LOW

### Scores

- Child Experience: 100/100
- Parent Experience: 100/100
- Teacher Experience: 100/100
- Engineering: 100/100
- SEO: 100/100
- Accessibility: 100/100

### Missing sections

- None

### Recommendations

- Minor polish only — align with Mini Weather Station depth where applicable.

## Build a Low-Voltage Smart Power Strip Simulator (`esp32-smart-power-strip`)

**Type:** golden · **Overall score:** 100/100 · **Priority:** LOW

### Scores

- Child Experience: 100/100
- Parent Experience: 100/100
- Teacher Experience: 100/100
- Engineering: 100/100
- SEO: 100/100
- Accessibility: 100/100

### Missing sections

- None

### Recommendations

- Minor polish only — align with Mini Weather Station depth where applicable.

## Build an Educational Soil pH Monitor (`esp32-soil-ph-monitor`)

**Type:** golden · **Overall score:** 100/100 · **Priority:** LOW

### Scores

- Child Experience: 100/100
- Parent Experience: 100/100
- Teacher Experience: 100/100
- Engineering: 100/100
- SEO: 100/100
- Accessibility: 100/100

### Missing sections

- None

### Recommendations

- Minor polish only — align with Mini Weather Station depth where applicable.

## Build an Estimated UV Index Monitor (`esp32-uv-index-monitor`)

**Type:** golden · **Overall score:** 100/100 · **Priority:** LOW

### Scores

- Child Experience: 100/100
- Parent Experience: 100/100
- Teacher Experience: 100/100
- Engineering: 100/100
- SEO: 100/100
- Accessibility: 100/100

### Missing sections

- None

### Recommendations

- Minor polish only — align with Mini Weather Station depth where applicable.

## Build a Sound-Triggered Relay Demo (`esp32-voice-controlled-relay`)

**Type:** golden · **Overall score:** 100/100 · **Priority:** LOW

### Scores

- Child Experience: 100/100
- Parent Experience: 100/100
- Teacher Experience: 100/100
- Engineering: 100/100
- SEO: 100/100
- Accessibility: 100/100

### Missing sections

- None

### Recommendations

- Minor polish only — align with Mini Weather Station depth where applicable.

## Build a Water Leak Detector (`esp32-water-leak-detector`)

**Type:** golden · **Overall score:** 100/100 · **Priority:** LOW

### Scores

- Child Experience: 100/100
- Parent Experience: 100/100
- Teacher Experience: 100/100
- Engineering: 100/100
- SEO: 100/100
- Accessibility: 100/100

### Missing sections

- None

### Recommendations

- Minor polish only — align with Mini Weather Station depth where applicable.

## Build Your WiFi Robot Controller (`esp32-wifi-robot-controller`)

**Type:** golden · **Overall score:** 100/100 · **Priority:** LOW

### Scores

- Child Experience: 100/100
- Parent Experience: 100/100
- Teacher Experience: 100/100
- Engineering: 100/100
- SEO: 100/100
- Accessibility: 100/100

### Missing sections

- None

### Recommendations

- Minor polish only — align with Mini Weather Station depth where applicable.
