...
from cms_loader import load_home, load_home_recommendations, load_site_settings
...
YOUTUBE_URL = _cfg.get("youtube_url", "https://www.youtube.com/@esp32-engine-projects")
...
ADSENSE_PUBLISHER_ID = "ca-pub-1799305676376920"
...
def modern_card(
    ...
    slug = p.get("slug", "")
    desc = p.get("desc") or card_description(p)
    desc_html = ""
    if show_desc and desc:
        short = card_description({**p, "meta_description": desc}, 100)
...
def _is_home_golden(project: dict) -> bool:
    return str(project.get("format", "")).lower() == "golden" or bool(project.get("project"))


def _home_project_map(projects: list[dict]) -> dict[str, dict]:
    return {p.get("slug", ""): p for p in projects if _is_home_golden(p)}


def _home_card(project: dict, role: str = "", tags: list[str] | None = None, reason: str = "") -> str:
    proj = project.get("project") or {}
    slug = project.get("slug", "")
    desc = card_description(project, 118)
    meta = [proj.get("difficulty"), proj.get("estimated_time"), proj.get("recommended_age") or proj.get("age")]
    meta_html = "".join(f'<span class="home-mini-meta">{esc(m)}</span>' for m in meta if m)
    skills = tags or (proj.get("skills_practiced") or proj.get("learning_objectives") or [])[:4]
    skill_html = "".join(f"<span>{esc(skill)}</span>" for skill in skills[:4])
    role_html = f'\n  <p class="home-card-role">{esc(role)}</p>' if role else ""
    reason_html = f'  <p class="home-card-reason">{esc(reason)}</p>\n' if reason else ""
    return f"""<a class="home-learning-card" href="{site_href(f'projects/{slug}.html')}">{role_html}
  <h3>{esc(project_title(project) or slug)}</h3>
  <p>{esc(desc)}</p>
{reason_html}
  <div class="home-card-meta">{meta_html}</div>
  <div class="home-skill-tags">{skill_html}</div>
  <span class="home-card-link">Build project</span>
</a>"""


def home_learning_adventure(projects: list[dict]) -> str:
    by_slug = _home_project_map(projects)
    config = load_home_recommendations()
    audiences = config.get("audiences", {})
    goals = config.get("goals", {})
    matrix = config.get("recommendations", {})
    roles = config.get("labels", ["Best First", "Next Challenge", "Advanced Upgrade"])
    default_audience = next(iter(audiences), "student")
    default_goal = next(iter(goals), "first-esp32-project")

    def resolved_cards(audience: str, goal: str) -> list[dict]:
        cards = []
        used = set()
        for slug in ((matrix.get(audience) or {}).get(goal) or []):
            if slug not in by_slug or slug in used:
                continue
            project = by_slug[slug]
            proj = project.get("project") or {}
            cards.append({
                "role": roles[len(cards)] if len(cards) < len(roles) else "Recommended",
                "title": project_title(project) or slug,
                "desc": card_description(project, 118),
                "href": site_href(f"projects/{slug}.html"),
                "meta": [proj.get("difficulty", ""), proj.get("estimated_time", ""), proj.get("recommended_age", "") or proj.get("age", "")],
                "skills": (proj.get("skills_practiced") or proj.get("learning_objectives") or [])[:4],
                "slug": slug,
            })
            used.add(slug)
            if len(cards) == 3:
    ...
    data = {
        audience: {goal: resolved_cards(audience, goal) for goal in goals}
        for audience in audiences
    }

    def rows(audience: str, goal: str) -> str:
        return "".join(_home_card(by_slug[item["slug"]], item["role"]) for item in data.get(audience, {}).get(goal, []))

    fallback_sections = []
    for audience, audience_label in audiences.items():
        for goal, goal_label in goals.items():
            cards = rows(audience, goal)
            if cards:
                fallback_sections.append(
                    f"""<details class="home-fallback-combo"><summary>{esc(audience_label)}: {esc(goal_label)}</summary><div class="home-recommendation-grid">{cards}</div></details>"""
                )
    audience_buttons = "".join(
        f'<button type="button" class="home-choice-btn{" is-active" if key == default_audience else ""}" data-audience="{esc(key)}" aria-pressed="{"true" if key == default_audience else "false"}">{esc(label)}</button>'
        for key, label in audiences.items()
    )
    goal_buttons = "".join(
        f'<button type="button" class="home-choice-btn{" is-active" if key == default_goal else ""}" data-goal="{esc(key)}" aria-pressed="{"true" if key == default_goal else "false"}">{esc(label)}</button>'
  ...
  <div class="wrap">
    ...
    <div class="home-recommendation-grid" id="home-recommendations">{rows(default_audience, default_goal)}</div>
    <noscript><section class="home-noscript" aria-labelledby="home-noscript-heading"><h3 id="home-noscript-heading">All recommendations</h3>{"".join(fallback_sections)}</section></noscript>
  </div>
  <script type="application/json" id="home-recommendation-data">{json.dumps(data, ensure_ascii=False)}</script>
</section>"""


def home_start_here(projects: list[dict]) -> str:
    by_slug = _home_project_map(projects)
    config = load_home_recommendations()
    cards = []
    for item in config.get("start_here", [])[:3]:
        slug = item.get("slug", "")
        if slug in by_slug:
            cards.append(_home_card(by_slug[slug], reason=item.get("reason", "")))
    return f"""<section class="home-start-here" aria-labelledby="home-start-heading">
  <div class="wrap"><p class="home-section-eyebrow">Start Here</p><h2 id="home-start-heading">Three safe beginner projects</h2><div class="home-recommendation-grid">{"".join(cards)}</div></div>
</section>"""


def home_popular_paths(projects: list[dict]) -> str:
    by_slug = _home_project_map(projects)
    cards = []
    for path in load_home_recommendations().get("learning_paths", []):
...
def home_build_experiment_upgrade(projects: list[dict]) -> str:
    by_slug = _home_project_map(projects)
    steps = load_home_recommendations().get("build_experiment_upgrade", {})
    if not steps:
        return ""
    cards = []
...
def home_learning_finder_js() -> str:
    return """<script>
document.addEventListener('DOMContentLoaded', function () {
  const dataEl = document.getElementById('home-recommendation-data');
  const out = document.getElementById('home-recommendations');
  if (!dataEl || !out) return;
  const data = JSON.parse(dataEl.textContent || '{}');
  let activeAudience = 'student';
  let activeGoal = 'first-esp32-project';
  function render() {
    const audienceRows = data[activeAudience] || data.student || {};
    const rows = audienceRows[activeGoal] || [];
    out.innerHTML = rows.slice(0, 3).map((p) => {
      const meta = (p.meta || []).filter(Boolean).map((m) => `<span class="home-mini-meta">${m}</span>`).join('');
      const skills = (p.skills || []).slice(0, 4).map((s) => `<span>${s}</span>`).join('');
      return `<a class="home-learning-card" href="${p.href}"><p class="home-card-role">${p.role}</p><h3>${p.title}</h3><p>${p.desc}</p><div class="home-card-meta">${meta}</div><div class="home-skill-tags">${skills}</div><span class="home-card-link">Build project</span></a>`;
    }).join('');
  }
  document.querySelectorAll('[data-goal]').forEach((btn) => btn.addEventListener('click', () => {
    activeGoal = btn.dataset.goal || activeGoal;
    document.querySelectorAll('[data-goal]').forEach((b) => { b.classList.toggle('is-active', b === btn); b.setAttribute('aria-pressed', b === btn ? 'true' : 'false'); });
    render();
  }));
  document.querySelectorAll('[data-audience]').forEach((btn) => btn.addEventListener('click', () => {
    activeAudience = btn.dataset.audience || activeAudience;
    document.querySelectorAll('[data-audience]').forEach((b) => { b.classList.toggle('is-active', b === btn); b.setAttribute('aria-pressed', b === btn ? 'true' : 'false'); });
    render();
  }));
});
</script>"""
    ...
    def project_rows():
        ...
        for p in _pick_by_slug(projects, TOP_PROJECT_SLUGS):
            ...
            rows.append(
                ...
                f"{image_html}"
                f'<span class="v3-pick-row-main">'
                f'{feat}<strong>{esc(project_title(p))}</strong>'
...
def home_v3_progress(counts: dict[str, int] | None = None) -> str:
    ...
    STATS = [
        (str(counts["golden_projects"]), "Projects"),
        (str(counts["missions"]), "Academy Missions"),
        (str(counts["components"]), "Components"),
        ("100%", "Free"),
    ]
    stats_html = "".join(
